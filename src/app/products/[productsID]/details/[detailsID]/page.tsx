...
import SubFooter from '@/src/app/components/subFooter'
...
export default async function Page({
  ...
  const description = (props:string) => {
    const descriptionFormated = props.split('\n').map(str => <p key={selectedItem.id} className="mb-4">{str}</p>);
  ...
  const inclusion = selectedItem.includes.map((inclusion) => {
    return (
        <p key={inclusion.item} className='text-primaryDark text-subTitle font-bold'>{`${inclusion.quantity}x`}
